  ...
  return (
    <div className="w-full">
      <div className="mb-2 flex items-center justify-between px-1">
        <span className="font-mono text-[0.62rem] uppercase tracking-[0.18em] text-muted">抽取时间线</span>
        <span className="font-mono text-[0.62rem] uppercase tracking-[0.14em] text-muted">共 {totalPaid} 抽 / 不计免费</span>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: H }}>
        <defs>
          {markers.map((m, i) => (
            <clipPath key={`clip-${i}`} id={`clip-${i}`}>
              <rect x={xPos(m.paidIndex) - 14} y={44 + i * rowH + 2} width={28} height={28} />
            </clipPath>
          ))}
        </defs>

        {/* Zone backgrounds */}
        <rect x={padL} y={barY - 3} width={Math.min(barW, (80 / maxX) * barW)} height={barH + 6} fill="#22c55e" opacity={0.08} />
        {maxX > 80 && (
          <rect x={padL + (80 / maxX) * barW} y={barY - 3} width={Math.min(barW - (80 / maxX) * barW, (40 / maxX) * barW)} height={barH + 6} fill="#f59e0b" opacity={0.08} />
        )}
        {maxX > 120 && (
          <rect x={padL + (120 / maxX) * barW} y={barY - 3} width={barW - (120 / maxX) * barW} height={barH + 6} fill="#dc2626" opacity={0.08} />
        )}

        {/* Main bar */}
        <rect x={padL} y={barY} width={barW} height={barH} fill="var(--text-muted)" opacity={0.2} />
        ...
        {markers.map((m, i) => {
          ...
          return (
            <g key={`${m.record.record_uid}-${m.paidIndex}`}>
              ...
              <circle cx={x} cy={dotY} r={6} fill={typeColor} stroke="var(--panel-bg)" strokeWidth={2} />

              <rect x={x - 110} y={labelY} width={220} height={rowH - 8} fill="var(--panel-strong)" stroke="var(--rule)" strokeWidth={1} />
      ...
      <div className="mt-2 flex justify-center gap-4 text-[10px] text-muted">
        <span className="flex items-center gap-1"><span className="inline-block h-2 w-2 bg-[#ef4444]" /> 6★UP</span>
        <span className="flex items-center gap-1"><span className="inline-block h-2 w-2 bg-[#f97316]" /> 6★</span>
        <span className="flex items-center gap-1"><span className="inline-block h-2 w-2 bg-[#f59e0b]" /> 5★</span>
      </div>
    </div>
  );
});
